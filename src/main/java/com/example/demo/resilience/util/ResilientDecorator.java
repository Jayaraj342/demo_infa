...
public class ResilientDecorator {
    ...
    public static class SupplierDecorator<T> {
        ...
        public SupplierDecorator<T> withThreadPoolBulkhead(ThreadPoolBulkhead threadPoolBulkhead) {
            ...
            this.supplier = () -> {
                try {
                    ...
                    throw new RuntimeException("ThreadPoolBulkhead execution failed", e);
                }
            };
            return this;
        }
                ...
                try {
                    return timeLimiter.executeFutureSupplier(
                            () -> CompletableFuture.supplyAsync(
                                    ContextPropagator.decorateSupplier(ResiliencePolicyRegistry.INSTANCE.getContextPropagators(), currentSupplier)
                            )
                    );
                } catch (Exception e) {
        ...
        public SupplierDecorator<T> withFallback(Supplier<T> fallbackSupplier) {
            Supplier<T> currentSupplier = this.supplier;
            this.supplier = () -> {
                try {
